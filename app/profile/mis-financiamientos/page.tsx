...
export default function MisFinanciamientosPage() {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#eef2ff_48%,#f8fafc_100%)] px-4 py-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900">Mis Financiamientos</h1>
            <p className="text-sm text-slate-500 mt-1">Sigue el estado de tus solicitudes de crédito vehicular.</p>
          </div>
        </div>
        ...
        {financiamientos.length === 0 && !error ? (
          <Card className="rounded-[2rem] border-dashed border-slate-300 bg-white/50 text-center shadow-none">
            <CardContent className="flex flex-col items-center justify-center p-12">
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100">
                <Banknote className="h-8 w-8 text-blue-600" />
              </div>
              <h3 className="text-xl font-bold text-slate-900">Aún no tienes solicitudes</h3>
          ...
          <div className="grid gap-6">
            {financiamientos.map((fin) => (
              <Card key={fin.id} className="overflow-hidden rounded-[2rem] border-slate-200 bg-white shadow-lg transition-shadow hover:shadow-xl">
                <div className="flex flex-col sm:flex-row">
                  {/* Imagen del vehículo */}
                  <div className="relative h-48 w-full sm:h-auto sm:w-64 shrink-0 bg-slate-100">
                    {fin.imagen ? (
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-auto rounded-2xl bg-slate-50 p-4 border border-slate-100">
                      <div>
                        <p className="text-xs font-medium text-slate-500 flex items-center gap-1">
                          <Banknote className="h-3.5 w-3.5" /> Monto financiado
                        </p>
                        <p className="text-lg font-bold text-slate-900">{formatCurrency(fin.monto_financiado)}</p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-slate-500 flex items-center gap-1">
                          <Calendar className="h-3.5 w-3.5" /> Plazo
                        </p>
                        <p className="text-lg font-bold text-slate-900">{fin.plazo_meses} meses</p>
                      </div>
                    </div>
                  </div>
                </div>
